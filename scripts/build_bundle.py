...
data = {n: json.loads((ROOT / "data" / f"{n}.json").read_text()) for n in ("manifest", "periode", "insiden", "kasus_resmi", "koordinat")}
...
js = re.sub(r'return Promise\.all\(\["periode", "insiden", "kasus_resmi", "koordinat"\]\.map\(function \(n\) \{\s*return fetch\("data/" \+ n \+ "\.json"\)\.then\(function \(r\) \{ return r\.json\(\); \}\);\s*\}\)\);',
            'return Promise.resolve(["periode","insiden","kasus_resmi","koordinat"].map(function(n){return window.EPPOS_DATA[n];}));', js)
